...
# 	Compile 4-state model
	${NVCC} -o BeagleCUDA_kernels.ptx --default-stream per-thread -src-in-ptx -DCUDA -DSTATE_COUNT=4 \
...
#
	for s in $STATE_COUNT_LIST; do \
		...
		${NVCC} -o BeagleCUDA_kernels.ptx --default-stream per-thread -src-in-ptx -DCUDA -DSTATE_COUNT=$s \
...
#
# 	Compile 4-state model
	${NVCC} -o BeagleCUDA_kernels.ptx --default-stream per-thread -src-in-ptx -DCUDA -DSTATE_COUNT=4 -DDOUBLE_PRECISION \
		$srcdir/kernels4.cu ${NVCCFLAGS} -DHAVE_CONFIG_H ${INCLUDE_DIRS} || { \rm BeagleCUDA_kernels.h; exit; }; \
...
#
	for s in $STATE_COUNT_LIST; do \
		...
		${NVCC} -o BeagleCUDA_kernels.ptx --default-stream per-thread -src-in-ptx -DCUDA -DSTATE_COUNT=$s -DDOUBLE_PRECISION \
			$srcdir/kernelsX.cu ${NVCCFLAGS} -DHAVE_CONFIG_H ${INCLUDE_DIRS} || { \rm BeagleCUDA_kernels.h; exit; }; \
