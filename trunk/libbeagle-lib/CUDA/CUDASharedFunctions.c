...
void storeGPURealMemoryArray(REAL* toGPUPtr,
                             REAL* fromGPUPtr,
                             int length) {
    SAFE_CUDA(cudaMemcpy(toGPUPtr, fromGPUPtr, SIZE_REAL * length, cudaMemcpyDeviceToDevice),
              toGPUPtr);
}
...
void storeGPUIntMemoryArray(INT* toGPUPtr,
                            INT* fromGPUPtr,
                            int length) {
    SAFE_CUDA(cudaMemcpy(toGPUPtr, fromGPUPtr, SIZE_INT * length, cudaMemcpyDeviceToDevice),
              toGPUPtr);
}
